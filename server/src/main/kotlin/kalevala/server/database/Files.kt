...
import kalevala.common.models.ImageVersion
import kalevala.common.models.ParticipantFile
import kalevala.common.models.StaticFile
import kalevala.common.models.participants.EthnoTourParticipant
...
import kalevala.server.scaleImageByRect
...
fun getOpenParticipantInfo(formType: FormType, fileId: Int): List<String> {
    ...
    transaction {
        ...
        when (formType) {
            ...
            FormType.EthnoMotive -> {
                if (participant[participantTable.get<String>("hasGroup")] == "Single") {
                    list += participant[participantTable.get<String>("surname")] + " " +
                        participant[participantTable.get<String>("name")] + " " +
                        participant[participantTable.get("patronymic")]
                    list += participant[participantTable.get<String>("age")].toString()
                } else {
                    list += participant[participantTable.get<String>("group")]
                    list += ""
                }
