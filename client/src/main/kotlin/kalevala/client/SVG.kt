...
open class LINE(initialAttributes: Map<String, String>, override val consumer: TagConsumer<*>) : HTMLTag("line", consumer, initialAttributes, null, false, false), HtmlBlockTag {
...
open class POLYGON(initialAttributes: Map<String, String>, override val consumer: TagConsumer<*>) : HTMLTag("polygon", consumer, initialAttributes, null, false, false), HtmlBlockTag {
    var points: String by StringAttribute()
    var fill: String by StringAttribute()
    var stroke: String by StringAttribute()
}

inline fun RDOMBuilder<SVG>.line(classes: String? = null, block: RDOMBuilder<LINE>.() -> Unit): ReactElement = tag(block) { LINE(attributesMapOf("class", classes), it) }

inline fun RDOMBuilder<SVG>.line(x1: Number, y1: Number, x2: Number, y2: Number, color: Color, classes: String? = null, block: RDOMBuilder<LINE>.() -> Unit) =
    line {
...
inline fun RDOMBuilder<SVG>.polygon(classes: String? = null, block: RDOMBuilder<POLYGON>.() -> Unit): ReactElement = tag(block) { POLYGON(attributesMapOf("class", classes), it) }
